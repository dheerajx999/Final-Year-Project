    ...
    success: function (response) {
      if (response.length > 0) {
        for (var userlist in response) {
          $("#orgStateId").append(
            $("<option></option>")
              .attr("value", response[userlist].num_state_id)
              .text(response[userlist].str_state_name)
          );
        }
      }
    },
...
function selectDistrict(stateId) {
  $("#orgDistId").find("option").remove().end();
    ...
    success: function (response) {
      if (response.length > 0) {
        ...
        for (var userlist in response) {
          $("#orgDistId").append(
            $("<option></option>")
              .attr("value", response[userlist].num_district_id)
              .text(response[userlist].str_district_name)
          );
        }
      }
    },
...
function checkUserExist(data) {
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response == true) {
        error_message(
          "This user name already present , please choose another one"
        );
        $("#userName").val("");
        $(id).val("");
    ...
    fields: {
      ...
      empNo: {
        validators: {
          ...
          regexp: {
            regexp: /^[a-zA-Z0-9.,\-\/\@\+\s &()]+$/,
            message:
              "Employee No should be less than 20 characters and only alphabet,numbers, special characters(. , / () - & @ +) and white spaces are permitted",
          },
        },
      },
      ...
      orgName: {
        validators: {
          ...
          regexp: {
            regexp: /^[a-zA-Z0-9().\/,\s\-]+$/,
            message:
              "Only alphabet, numbers, special characters( / , . () - ) and white spaces are permitted",
          },
          stringLength: {
            min: 1,
            max: 250,
      ...
      contactPerDesg: {
        validators: {
          notEmpty: {
            message:
              "Contact Person Designation is required and can't be empty",
          },
      ...
      contactPersonMobileNo: {
        validators: {
          notEmpty: {
            message:
              "Contact Person Mobile number is required and can't be empty",
          },
          regexp: {
            regexp: /^[0-9\-]+$/,
            message: "Please enter valid numbers",
